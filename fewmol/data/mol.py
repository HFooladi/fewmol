...
def parse_args():
    ...
    parser.add_argument(
        "--smiles",
        type=str,
        default="O1C=C[C@H]([C@H]1O2)c3c2cc(OC)c4c3OC(=O)C5=C4CCC(=O)5",
        help="",
    )
...
def smiles2graph(
    smiles_string: str, removeHs: bool = True, reorder_atoms: bool = False
) -> Dict[str, np.ndarray]:
    """
    Converts SMILES string to graph Data object
    :input: SMILES string (str)
    :return: graph object (dict)
    """
...
def create_pyg_graph(graph: Dict, additional_node_files=[], additional_edge_files=[]) -> Data:
    ...
    if "graph_label" in graph.keys():
        if graph["graph_label"] is not None:
            assert isinstance(
                graph["graph_label"], torch.Tensor
            ), "graph_label should be a torch.Tensor"
            g.y = graph["graph_label"].to(torch.float32)
            del graph["graph_label"]

    return g


def create_pyg_graph_from_smiles(
    smiles_string: Union[str, List],
    removeHs: bool = True,
    ...
    for i, smiles in enumerate(smiles_string):
        ...
        graphs.append(graph)
        pyg_graphs.append(
            create_pyg_graph(graph, additional_node_files=[], additional_edge_files=[])
        )
